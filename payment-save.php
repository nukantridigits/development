...
Ext.define('kDesktop.transportation3.transpEdit.ferryPanel', {
	...
	constructor: function(config) {
		...
		const RolesHelper = helpers.roles
		if (RolesHelper.isFieldHidden(this.permissions, RolesHelper.RESOURCE_TRANSPORTATIONS, 'ferryman_name')) {
		...
		this.carsStore.on('load', function () {
			this.showSelectCarModalBtn.setText("Список машин (" + this.carsStore.getTotalCount() + ")")
		}, this)

		this.driversStore.on('load', function () {
			this.showSelectDriverModalBtn.setText("Список водителей (" + this.driversStore.getTotalCount() + ")")
		}, this)
										...
										items: [
													...
													const selectCarModal = Ext.create('car.selectCarModal', {
														...
														listeners: {
															onCarSelect: (component, { car = {} }) => {
																this.carIdFld.setValue(car?.id ?? '')
																this.carIdCaptionFld.setValue('')
																this.ferrycarFld.setValue(car?.ferrycar ?? '')
																this.ferrycarnumberFld.setValue(car?.ferrycarnumber ?? '')
																this.ferrycarppFld.setValue(car?.ferrycarpp ?? '')
																this.ferrycarppnumberFld.setValue(car?.ferrycarppnumber ?? '')
																this.ferryman_typets_strFld.setValue(car?.typets_str ?? '')
																this.ferryman_typetsFld.setValue(car?.typets ?? '')
															}
														}
													})
											...
											{ xtype: 'displayfield', width: 10, value: '&nbsp;', alias: 'ferryman_name' },
											{
												...
												value: !!this.parent.data.data.ferrycar_id
													? ''
													: '<font color="#ff0000"><b>Машина не прикреплена</b></font>',
												ref: 'carIdCaptionFld'
											}
										]
									},
										...
										items: [
													...
													const selectDriverModal = Ext.create('driver.selectDriverModal', {
														...
														listeners: {
															onDriverSelect: (component, { driver = {} }) => {
																this.driverIdFld.setValue(driver?.id ? parseInt(driver.id) : null)
																this.driverIdCaptionFld.setValue('')
																this.ferryfiodriverFld.setValue(driver?.fio ?? '')
																this.ferryphoneFld.setValue(driver?.phone ?? '')
																this.ferrypassportFld.setValue(driver?.passport ?? '')
															}
														}
													})
											...
											{ xtype: 'displayfield', width: 10, value: '&nbsp;', alias: 'ferryman_name' },
											{
												...
												value: !!this.parent.data.data.driver_id
													? ''
													: '<font color="#ff0000"><b>Водитель не прикреплен</b></font>',
							...
							},
							{
								...
								handler: () => {
									const modal = Ext.create('driver.modal', {
										...
										permissions: this.permissions,
										clientConfig: this.clientConfig,
										carrierId: this.parent.data.data.ferryman,
										listeners: {
											onCarUpdate: () => {
